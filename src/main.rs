...
trait UInts {const ELEMENT_BITS:usize;}
...
struct Bitys<ElementType: UInts> {
    bytes:Vec<ElementType>,
    set_bits:usize
}

impl<ElementType: UInts + BitField + Default + Clone + Copy> Bitys<ElementType> {
    fn bit_idx(bitdex:usize) -> usize {bitdex%ElementType::ELEMENT_BITS}
    fn elems_idx(bitdex:usize) -> usize {bitdex/ElementType::ELEMENT_BITS}
    fn missalignment(&self) -> usize {Self::bit_idx(self.set_bits)}
    fn get(&self, index:usize) -> bool {self.bytes[Self::elems_idx(index)].get_bit(Self::bit_idx(index))}
    fn new_empty_elem(&mut self) {self.bytes.push(ElementType::default())}
    fn new_empty_elems(&mut self, empty_elems:usize) {self.bytes.resize(self.bytes.len()+empty_elems,ElementType::default())}
    fn set(&mut self, bitdex:usize, val:bool) {
    ...
    fn new() -> Self{ Self{bytes:Vec::new(), set_bits:0}  }
